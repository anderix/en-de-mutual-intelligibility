# Germanic cognates without a named shift

Pairs where the kinship to English is obvious by eye or ear, but no single sound-shift rule cleanly captures the relationship. These are the bedrock of mutual intelligibility — basic Germanic vocabulary that survived in both languages with only small mutations.

When a word fits a clean sound-shift pattern (*Wasser/water*, *Buch/book*, *Bruder/brother*), it lives in the relevant shift file. This file holds the rest.

## Pairs

### Body and family

**eye ↔ das Auge** *(A1)* — also in sound-equivalent.md
*Sie hat blaue Augen wie ihre Mutter.*

**hair ↔ das Haar** *(A1)*
*Lisa hat lange braune Haare.*

**hand ↔ die Hand** *(A1)* — also in identical-spelling-and-sound.md
*Er gibt mir zur Begrüßung die Hand.*

**son ↔ der Sohn** *(A1)*
*Mein Sohn ist sechs Jahre alt.*

**sister ↔ die Schwester** *(A1)*
The *sw-* in older English became *si-*; German kept the *Schw-*. Visible cognate.
*Meine Schwester wohnt in München.*

**man ↔ der Mann** *(A1)* — also in identical-spelling-and-sound.md
*Der Mann dort drüben ist mein Nachbar.*

### Home and everyday objects

**house ↔ das Haus** *(A1)* — also in sound-equivalent.md (au/ou pattern)
*Unser Haus hat einen kleinen Garten.*

**bread ↔ das Brot** *(A1)*
*Zum Frühstück esse ich Brot mit Marmelade.*

**glass ↔ das Glas** *(A1)*
*Ein Glas Wein, bitte.*

**shoe ↔ der Schuh** *(A1)*
The *sh* in English is *sch* in German — a consistent spelling difference rather than a sound shift.
*Diese Schuhe sind zu klein.*

**garden ↔ der Garten** *(A1)*
*Im Garten wachsen viele Blumen.*

**fire ↔ das Feuer** *(A1)*
Sound similarity: /faɪər/ vs /ˈfɔɪɐ/. Different vowels but same consonantal frame.
*Das Feuer wärmt den ganzen Raum.*

### Animals, food, drink

**fish ↔ der Fisch** *(A1)*
*Am Freitag essen wir oft Fisch.*

**hound ↔ der Hund** *(A1, semantic shift: Hund is generic dog, hound is a specific type)*
*Der Hund meiner Nachbarn heißt Max.*

**egg ↔ das Ei** *(A1)*
Sound: /ɛɡ/ vs /aɪ/. Cognate via Proto-Germanic *ajjaz* — English added the *-g* later, German kept the diphthong.
*Ein weiches Ei zum Frühstück, bitte.*

**beer ↔ das Bier** *(A1)*
*Ein Bier nach der Arbeit ist schön.*

**wine ↔ der Wein** *(A1, both from Latin vinum but parallel descent)*
*Der Wein aus Italien schmeckt hervorragend.*

### Nature and time

**land ↔ das Land** *(A1)* — also in identical-spelling
*Welches Land ist dein Heimatland?*

**wind ↔ der Wind** *(A1)* — also in identical-spelling
*Der Wind kommt aus dem Norden.*

**sun ↔ die Sonne** *(A1)*
*Die Sonne geht gerade auf.*

**winter ↔ Winter** *(implied A1 — common reference word; identical spelling)*

**morning / morrow ↔ morgen / Morgen** *(A1)*
The German *morgen* covers both "tomorrow" (adverb) and "morning" (noun, capitalized). English *morrow* is archaic; *morning* is the modern form.
*Morgen fahren wir in den Urlaub.*

**fever ↔ das Fieber** *(A1)*
*Das Kind hat seit gestern Fieber.*

**hunger ↔ der Hunger** *(A1)* — also in identical-spelling
*Nach dem langen Spaziergang habe ich Hunger.*

### High-frequency verbs

**have ↔ haben** *(A1)*
*Wir haben heute viel Zeit.*

**be (sein) ↔ sein** *(A1)*
*Ich bin heute sehr müde.*

**find ↔ finden** *(A1)*
*Ich kann meine Brille nicht finden.*

**bring ↔ bringen** *(A1)*
*Bringst du mir bitte ein Glas Wasser?*

**come ↔ kommen** *(A1)*
*Woher kommst du ursprünglich?*

**go ↔ gehen** *(A1)*
*Am Abend gehe ich gern spazieren.*

**say ↔ sagen** *(A1)*
*Was sagt der Arzt dazu?*

**see ↔ sehen** *(A1)*
*Siehst du den roten Wagen dort?*

**hear ↔ hören** *(A1)*
*Ich höre gern klassische Musik.*

**stand ↔ stehen** *(A1)*
*Der Tisch steht mitten im Zimmer.*

**sit ↔ sitzen** *(A1)*
The *-tz-* in German is the High German consonant shift t → z (see shift-t-to-ss-z.md), but the kinship is so visible it goes here too.
*Wir sitzen im Garten und reden.*

**lie ↔ liegen** *(A1)*
*Die Zeitung liegt auf dem Tisch.*

**lay ↔ legen** *(A1)*
*Leg bitte das Buch auf das Regal.*

**swim ↔ schwimmen** *(A1)*
*Am Wochenende schwimme ich im See.*

**fly ↔ fliegen** *(A1)*
*Morgen fliegen wir nach Spanien.*

**help ↔ helfen** *(A1)*
*Kannst du mir bei den Hausaufgaben helfen?*

**wash ↔ waschen** *(A1)*
*Ich wasche meine Hände vor dem Essen.*

**give ↔ geben** *(A1)*
*Bitte gib mir das Salz.*

**learn ↔ lernen** *(A1)*
*Seit einem Jahr lerne ich Deutsch.*

**hold ↔ halten** *(A1)*
*Der Bus hält hier nicht.*

**send ↔ senden** *(implied via Absender, A1)*
*Ich sende dir die Datei per E-Mail.*

**answer ↔ antworten** *(A1)*
*Sie antwortet auf meine Frage.*

**fare (travel) ↔ fahren** *(A1, semantic shift)*
Modern English *fare* is mostly archaic ("farewell," "wayfarer"); German *fahren* is the everyday verb for going by vehicle.
*Morgen fahre ich mit dem Zug nach Köln.*

**will (want) ↔ wollen** *(A1)*
English *will* shifted to a future-marker; German *wollen* still means "want."
*Wir wollen heute Abend Pizza essen.*

**may (like) ↔ mögen** *(A1)*
*Ich mag besonders italienisches Essen.*

**must ↔ müssen** *(A1)*
*Wir müssen früh aufstehen.*

**shall ↔ sollen** *(A1)*
*Soll ich dich vom Bahnhof abholen?*

**can ↔ können** *(A1)*
*Kannst du gut kochen?*

**know (a person) ↔ kennen** *(A1)*
Cognate with archaic English *ken*. German *kennen* means knowing a person or thing; *wissen* means knowing a fact. English collapsed both into *know*.
*Kennst du diesen Mann?*

**know (a fact) ↔ wissen** *(A1, cognate with English *wit*, *wise*)*
*Weißt du, wo die Bibliothek ist?*

**love ↔ lieben** *(A1)*
*Die Kinder lieben ihre Eltern.*

**live ↔ leben** *(A1)*
*Meine Großeltern leben auf dem Land.*

### High-frequency adjectives and adverbs

**young ↔ jung** *(A1)*
*Er ist noch sehr jung für diese Arbeit.*

**new ↔ neu** *(A1)*
*Meine Schuhe sind ganz neu.*

**long ↔ lang** *(A1)*
*Der Weg zur Schule ist lang.*

**free ↔ frei** *(A1)*
*Ist dieser Stuhl noch frei?*

**friend ↔ der Freund** *(A1)*
The German *Fr-* with *-eu-* corresponds to English *fr-* with *-ie-*; recognizable kinship.
*Tom ist mein bester Freund seit Jahren.*

**right (direction) ↔ rechts** *(A1)*
*An der Kreuzung gehst du rechts.*

**right (correct) ↔ richtig** *(A1)*
*Deine Antwort ist richtig.*

**oft (often) ↔ oft** *(A1)*
*Wir sehen uns oft am Wochenende.*

**more ↔ mehr** *(A1)*
*Möchtest du noch mehr Kaffee?*

**most ↔ meist-** *(A1)*
*Die meisten Studenten kommen aus Europa.*

**mine ↔ mein** *(A1)*
*Das ist mein neues Fahrrad.*

**all ↔ all-** *(A1)*
*Alle Fenster sind geöffnet.*

**alone ↔ allein** *(A1)*
*All + ein = al + one = alone.* Same logic in both languages.
*Sie lebt allein in einer kleinen Wohnung.*

**small (semantic) ↔ klein** *(A1)*
*Das Mädchen hat einen kleinen Hund.*

**large (semantic) ↔ groß** *(A1)*
Cognate with *great*; modern English semantic split.
*Berlin ist eine große Stadt.*

**again ↔ noch / wieder (in compounds)** *(A1)*
*wieder* in *wiederholen* (repeat), *Wiedersehen* (goodbye), *Wiederhören* (goodbye on phone). *Wieder = with-the-again* idea, cognate with English *with-er* and ultimately related to *again*.

**well, healthy ↔ gut** *(A1)* — also in shift-d-to-t (good/gut)

**rain ↔ Regen** *(A1)*
*Bei diesem Regen bleibe ich lieber zu Hause.*

**shine ↔ scheinen** *(A1)*
*Die Sonne scheint den ganzen Tag.*

**weather ↔ das Wetter** *(A1)* — also in shift-th-to-d
*Gestern hatten wir schlechtes Wetter.*

### A2 additions

**build ↔ bauen** *(A2, verb)*
*Wir bauen ein neues Haus am Stadtrand.*

**think ↔ denken** *(A2, verb)*
The *-nk-* cluster is preserved in both languages.
*Ich denke, du hast recht.*

**feel ↔ fühlen** *(A2, verb)*
*Wie fühlst du dich heute?*

**mean ↔ meinen** *(A2, verb)*
The noun *die Meinung* (opinion) appears in suffix-ung.md.
*Was meinst du mit dieser Frage?*

**try / seek ↔ versuchen** *(A2, verb)*
*ver-* prefix added to *suchen* (see shift-k-to-ch.md for *suchen / seek*).
*Ich versuche es morgen noch einmal.*

**lose ↔ verlieren** *(A2, verb)*
Cognate via Proto-Germanic; related to English "forlorn" (the past participle *verloren* is visible in that pair).
*Ich habe meine Schlüssel verloren.*

**wait ↔ warten** *(A2, verb)*
Cognate with English "ward" and "watch" (same Proto-Germanic root *wardōn-*, "to watch over").
*Warte bitte einen Moment auf mich.*

**neck / throat ↔ der Hals** *(A2)*
Cognate with archaic English *halse*. Modern English replaced it with *neck*.
*Mir tut der Hals weh.*

**forest / woods ↔ der Wald** *(A2)*
Cognate with English *wold* and *weald* (both archaic, preserved in place names like *the Weald*).
*Im Herbst gehe ich gern im Wald spazieren.*

**cat ↔ die Katze** *(A2)*
Both from Late Latin *catta*, which both languages absorbed early enough that they feel native.
*Unsere Katze schläft fast den ganzen Tag.*

**hard ↔ hart** *(A2, adjective)* — also fits shift-d-to-t
Final *d* devoices to *t*. Placed here because the kinship is immediate even without the shift rule.
*Dieses Brot ist schon sehr hart.*

**cold ↔ kalt** *(A2, adjective)* — also fits shift-d-to-t
*Heute Morgen ist es sehr kalt.*

**shall ↔ sollen** — already in A1 entries above.

**carry / wear ↔ tragen** *(A2, verb)*
Cognate with English *drag* (d → t shift initially). Semantic split: German kept "carry" and added "wear"; English kept "drag" for pulling and developed "carry" and "wear" separately.
*Er trägt heute einen schwarzen Anzug.*

**meet ↔ treffen** *(A2, verb)*
Cognate via Proto-Germanic; English "meet" and German "treffen" share a root meaning "to come upon."
*Wir treffen uns heute Abend im Café.*

## Note

This file overlaps with several other files (shift files mainly). When a word fits a clean shift rule, the primary entry is in the shift file; we cross-reference back here. The intent of this file is to be the catch-all for "you can see the kinship" cases that don't have a named rule — plus words where the kinship is so visible that listing them here earns its keep even if they also fit a shift rule.

This is also the file that proves the deepest principle: English and German share so much basic vocabulary that any English speaker, given a one-page primer on a few sound shifts and a few hundred everyday words, has *thousands* of effective German recognitions available.
